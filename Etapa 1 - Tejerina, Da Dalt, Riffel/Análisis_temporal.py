# -*- coding: utf-8 -*-
"""
Created on Wed Aug 16 15:32:01 2023

@author: TEJERINA, ROCIO
...
#%% Graficación de las señales temporales
...
fig, axes = plt.subplots(len(classmap), 3, figsize=(20, 20),squeeze=False)
...
for gesture_name in classmap:                           # Se recorre cada gesto
    for capture in range(int(len(x))):                  # Se recorre cada renglón de las matrices
        if (x[capture, N] == gesture_name):             # Si en el último elemento se detecta la etiqueta correspondiente
            ...
            x_volt= 0.3 * x[capture, 0:N]
            y_volt= 0.3 * y[capture, 0:N]
            z_volt= 0.3 * z[capture, 0:N]
            axes[gesture_name][0].plot(t, x_volt, label="Trial {}".format(trial_num))
            axes[gesture_name][1].plot(t, y_volt, label="Trial {}".format(trial_num))
            axes[gesture_name][2].plot(t, z_volt, label="Trial {}".format(trial_num))
...
# Se le da formato a los ejes de cada gráfica
    ...
    axes[gesture_name][0].set_ylabel('Aceleración [V]', fontsize=10)
    ...
    axes[gesture_name][1].set_ylabel('Aceleración [V]', fontsize=10)
    ...
    axes[gesture_name][2].set_ylabel('Aceleración [V]', fontsize=10)
