...
from ScriptsPython import process_data
from ScriptsPython import funciones_fft as fft
...
for gesto in classmap:
    ...
    # Si en el último elemento se detecta la etiqueta correspondiente                
        if (x[registro, N] == gesto):                       
            ...
            axes[0].set_xticks(np.arange(xmin,xmax,step))
            ...
            axes[1].set_xticks(np.arange(xmin,xmax,step))
            ...
            axes[2].set_xticks(np.arange(xmin,xmax,step))
            
    trial_num = 0
    plt.tight_layout()
    plt.show()

# Ahora para cada gesto buscamos el máximo valor en el total de repeticiones y con esto luego calculamos
...
for gesto in classmap:
    ...
    print("\t Aceleración X:")
    print("\t \t Frecuencia:",fc,"[Hz] -> Amplitud:",max_x_,"[mV] -> Atenuación requerida:",-20*np.log10(deltaV/max_x_),"[dB]\n")
    print("\t \t Frecuencia:",f_x ,"[Hz] -> Amplitud:",max_x ,"[mV] -> Atenuación requerida:",-20*np.log10(deltaV/max_x), "[dB]\n")
    print("\t Aceleración Y:")
    print("\t \t Frecuencia:",fc,"[Hz] -> Amplitud:",max_y_,"[mV] -> Atenuación requerida:",-20*np.log10(deltaV/max_y_),"[dB]\n")
    print("\t \t Frecuencia:",f_y ,"[Hz] -> Amplitud:",max_y ,"[mV] -> Atenuación requerida:",-20*np.log10(deltaV/max_y), "[dB]\n")
    print("\t Aceleración Z:")
    print("\t \t Frecuencia:",fc,"[Hz] -> Amplitud:",max_z_,"[mV] -> Atenuación requerida:",-20*np.log10(deltaV/max_z_),"[dB]\n")
    print("\t \t Frecuencia:",f_z ,"[Hz] -> Amplitud:",max_z ,"[mV] -> Atenuación requerida:",-20*np.log10(deltaV/max_z), "[dB]\n")
